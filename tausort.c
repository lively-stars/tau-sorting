...
void input(void){
 ...
 fprintf(stdout,"Reading Background File\n");
 ...
 fprintf(stdout,"Reading ODF File\n");
 fp = fopen("./p00big3.bdf","r");
 
 tab_T[0] = log(Tmin);
 delta_logT = (log(Tmax)-log(Tmin))/(NT-1);
 inv_delta_logT = 1.0/delta_logT;
 for (int tt =1;tt<NT-1;tt++)
   tab_T[tt] = tab_T[tt-1] + delta_logT;
 tab_T[NT-1] = log(Tmax);


 tab_p[0] = log(pmin);
 delta_logp = (log(pmax)-log(pmin))/(Np-1);
 inv_delta_logp = 1.0/delta_logp;
 for (int pp=1;pp<Np-1;pp++)
   tab_p[pp] = tab_p[pp-1] + delta_logp;
 tab_p[Np-1] = log(pmax);
 ...
 fprintf(stdout,"Reading Continuum Files\n");
 fp = fopen("./continuumabs.dat","r");
 fp2 = fopen("./continuumscat.dat","r");
 ...
 for (int lam=0; lam<Nlam; lam++)
   for (int t=0; t<NT; t++)
     for (int p=0; p<Np; p++){
       ...
       acont_pT[lam][t][p] = kcont_pT[lam][t][p]+scont_pT[lam][t][p]*cont_scatter;
 }
}
  ...
  for (nu=0; nu<=Nnu-1; nu++)
  {
    ...
    for (j=0; j<=NT-1; j++)
    {
      B[nu][j] = 2.e0*H/(pow(Cvac,2)) * pow(numid[nu],3) * 
        1.e0/(exp(H*numid[nu]/(k_B * exp(tab_T[j])) ) - 1.e0) ; 

      dBdT[nu][j] = 2.e0*pow(H,2)/(k_B*pow(Cvac,2))
        *pow(numid[nu],4)/pow(exp(tab_T[j]),2)
        *1.e0/( exp(H*numid[nu]/(k_B * exp(tab_T[j])) )
        +exp(-1.e0*H*numid[nu]/(k_B * exp(tab_T[j])) )
        -2.e0 );
     }  
   }
      ...
      if ((lgT >= tab_T[l]) && (lgT < tab_T[l+1]))
      {
        for (m=1; m<=Np-2; m++)
        {
          if ((lgp >= tab_p[m]) && (lgp < tab_p[m+1]))
          {
            for (nu=0; nu<=Nnu-1; nu++)
            {
              acont[k][nu]= inv_delta_logT*inv_delta_logp*
                ((tab_T[l+1]-lgT)* (tab_p[m+1]-lgp) * log10(acont_pT[nu][l][m])  
              + (lgT- tab_T[l])*( tab_p[m+1]-lgp) * log10(acont_pT[nu][l+1][m])
              + (tab_T[l+1]-lgT)*(lgp-tab_p[m]) * log10(acont_pT[nu][l][m+1])
              +  (lgT- tab_T[l])*(lgp-tab_p[m]) * log10(acont_pT[nu][l+1][m+1])); 
              acont[k][nu] = exp(TENLOG*acont[k][nu]);
             
              for (bin=0; bin<=Nbin-1; bin++){
                line_opacity[nu][bin][k] = inv_delta_logT*inv_delta_logp*
  ...
  for (int t=0; t<=NT-1; t++){
    for (int p=0; p<=Np-1; p++){
      for (int band=0; band<=Nbands-1; band++){
        ...
        tau_i = kap_ro[band][t][p]*exp(tab_p[p])/2.74e4;
      ...
      //fprintf(stdout,"T %d P %d kap_mean %e\n",t,p,kap_mean[Nbands-1][t][p]);
    }
    for (int band=0; band<=Nbands-1; band++)
