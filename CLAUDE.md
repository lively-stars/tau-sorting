...
uv run python tausort.py convert-model models/G_SSD models/G_SSD.dat        # binary STAGGER model -> G2_1D.dat ASCII format
...
- **`models/`** — STAGGER 1D model atmospheres (`F/G/K/M_SSD`, binary float32) used by `compare_Qrad_from_kappa.py`, `qrad_core.py`, and the webapp. `models/{F,G,K,M}_SSD.dat` are ASCII versions in G2_1D.dat's `z ρ p T` format (via `tausort.py convert-model`), usable directly as `--atm`.
