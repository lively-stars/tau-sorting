...
def planck_function(wavelength: NDArray[np.float64], temperature: np.float64) -> NDArray[np.float64]:
    """
    ...
    B = 2 * H * CVAC**2 / wl_cm**5 / (np.exp(H * CVAC / wl_cm / K_B / temperature) - 1)

    return B


def planck_derivative(wavelength: NDArray[np.float64], temperature: NDArray[np.float64]) -> NDArray[np.float64]:
    """
...
def plot_planck_and_derivatives():
    ...
    for i, T_val in enumerate(T_extra):
        ...
        ax.plot(wavelengths, B_extra, "--", color=colors_extra[i], linewidth=1.5, label=f"T={T_val} K")
    ...
    if True:
        ...
        for i, (idx, wl) in enumerate(zip(indices, selected_wavelengths)):
            ...
            # Plot the point
            ax.plot(wl, B_val, "o", color=colors[i], markersize=8, label=f"λ={wl:.0f} nm")
            ...
            ax.plot(
                wl_range, tangent_analytic, ":", color=colors[i], alpha=0.8, linewidth=2, label="Analytic derivative"
            )

    ax.set_xlabel("Wavelength (nm)", fontsize=12)
    ax.set_ylabel("Planck Function [erg/s/cm²/ster/cm]", fontsize=12)
    ax.set_title("Planck Function at Different Temperatures", fontsize=14)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=8, loc="best")

    plt.tight_layout()
    plt.savefig("planck_derivatives.png", dpi=150)
    console.print("[green]Plot saved to planck_derivatives.png[/green]")
    plt.show()
