...
for i, (t_idx, p_idx) in enumerate(zip(random_t_indices, random_p_indices)):
    print(f"  {i + 1}. T={T[t_idx]:.1f} K, P={P[p_idx]:.2e} dyn/cm², log10(P)={P[p_idx]:.2f}")
...
for idx, (t_idx, p_idx) in enumerate(zip(random_t_indices, random_p_indices)):
    ...
    ax.set_title(f"T={T[t_idx]:.0f} K, log₁₀(P)={P[p_idx]:.1f}", fontsize=10, fontweight="bold")
    ax.grid(True, alpha=0.3, which="both")
    ax.set_xlim(wavelength_centers_nm.min(), wavelength_centers_nm.max())

    # Add median line
    median_kappa = np.median(kappa_tp, axis=1)
    ax.semilogy(wavelength_centers_nm, median_kappa, "r-", linewidth=2, alpha=0.8, label="Median" if idx == 0 else None)

# Add legend to first subplot
axes[0].legend(fontsize=7, loc="upper right", ncol=2)

plt.suptitle(
    f"Opacity Distribution Functions at 10 Random (T, P) Points\n"
...
for idx, (t_idx, p_idx) in enumerate(zip(random_t_indices, random_p_indices)):
    ...
    kappa_tp = 10.0 ** (odf_tp / 1000.0)
    print(
        f"  Sample {idx + 1}: κ = {kappa_tp.min():.2e} - {kappa_tp.max():.2e} cm²/g "
